...
public class IfMetaAction extends TSLAction {
    ...
    public void perform(ListAccessor<TSLToken> arguments, TSLContext context) {
        ...
        for (List<TSLToken> part : parts) {
            List<List<TSLToken>> statement = TSLCollectionUtils.splitBy(part, false, token -> token instanceof TSLPlainWord
    ...
    private List<List<TSLToken>> splitParts(ListAccessor<TSLToken> arguments, boolean includeDelimiter) {
        return TSLCollectionUtils.splitBy(arguments, includeDelimiter, token -> {
